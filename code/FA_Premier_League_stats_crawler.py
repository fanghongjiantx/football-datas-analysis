...
#提取英超历年排名,保存html数据,提取其中排名到xlsx文件中
def get_FAstats(time):
    # 目标URL
    url = "https://fbref.com/en/comps/9/"+str(time)+"-"+str(time+1)+"/"+str(time)+"-"+str(time+1)+"-Premier-League-Stats"
    html_loca='D:/football/stats/html/FA_Premier/premier_league_table_'+str(time)+"-"+str(time+1)+'.html'
    xlsx_loca='D:/football/stats/data/FA_Premier/'+str(time)+"-"+str(time+1)+'_data.xlsx'
    # 发起请求
...
def count_FAteams():
    # 文件夹路径 (放置你的xlsx文件)
    folder_path = 'D:/football/stats/data/FA_Premier/'
    ...
    output_df.to_excel('element_count.xlsx', index=False)
...
#寻找某支球队未在英超的赛季
def find_team_notin_FAp(team_name):
     # 文件夹路径 (放置你的xlsx文件)
    folder_path = 'D:/football/stats/data/FA_Premier/'
...
def run():
    # for t in range(1992,2023):
    #     get_FAstats(t)
    #     time.sleep(20)

    # count_FAteams()

    find_team_notin_FAp("Manchester City")
